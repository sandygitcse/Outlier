    ...
    for i, data in enumerate(loader, 0):
        ...
        if unnorm:
            ...
            if args.n_quant>1:
                #import pdb;pdb.set_trace()
                batch_pred = []
                for i in range(args.n_quant):
                    batch_pred.append(norm.unnormalize(batch_pred_mu[..., i], ids=ids, is_var=False).unsqueeze(-1))
                batch_pred_mu = torch.cat(batch_pred,dim=-1)
            else:
