...
def parse_ett(dataset_name, N_input, N_output, t2v_type=None):
    df = pd.read_csv(DATA_DIRS+'/data/ETT/ETTm1.csv')
    ...
    # data_mask_train,data_mask_dev,data_mask_test = torch.zeros_like(data_train),torch.zeros_like(torch.tensor(data_dev),torch.zeros_like(data_test)
...
#    df = pd.read_csv('../Informer2020/data/ETT/ETTh1.csv').iloc[:n]
    ...
    df = pd.read_csv(DATA_DIRS+'data/ETT/ETTh1.csv')
    # Remove incomplete data from last day
    df = df[:-20]
...
def parse_smd(dataset_name, N_input, N_output, t2v_type=None):
    ...
    for fn_i in train_files[2:3]: # TODO Selecting 5 series is temp, change this
...
def parse_electricity(dataset_name, N_input, N_output, t2v_type=None):
    ...
    # )
    df_inject   = pd.read_csv(
        os.path.join(DATA_DIRS, 'data', 'electricity_load_forecasting_panama', 'electricity_global_point_0.3.csv')
    )
...
def parse_taxi(dataset_name, N_input, N_output, t2v_type=None):
    
    df = pd.read_csv(
        os.path.join(DATA_DIRS, 'data', 'taxi', 'NYC_Taxi.csv')
    )

    data = df[['value']].to_numpy().T
    data_mask = df[['label']].to_numpy().T   #### original anomalies
