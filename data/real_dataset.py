...
def parse_foodinflation(dataset_name, N_input, N_output, t2v_type=None):
    #df = pd.read_csv('data/electricity_load_forecasting_panama/continuous_dataset.csv')
    df = pd.read_csv(
        os.path.join(DATA_DIRS, 'data', 'synthetic', 'train_data.csv')
    )
    # df = pd.read_csv(
    ...
    feats = np.concatenate([feats_date])
    data = torch.tensor(data, dtype=torch.float)
    feats = torch.tensor(feats, dtype=torch.float)
...
def parse_synthetic(dataset_name, N_input, N_output, t2v_type=None):
    #df = pd.read_csv('data/electricity_load_forecasting_panama/continuous_dataset.csv')
    df = pd.read_csv(
        os.path.join(DATA_DIRS, 'data', 'synthetic','dataset', 'training_data_outlier.csv')
    )
    # df = pd.read_csv(
    #     os.path.join(DATA_DIRS, 'data', 'foodinflation', 'train_data.csv')
    # )
...
def parse_telemetry(dataset_name, N_input, N_output, t2v_type=None):
    # /mnt/cat/data/sandy/Forecasting/Telemanom/DILATE/data/telemetry/data/training_data
    trn_channel_paths = glob.glob(os.path.join(DATA_DIRS, 'data', 'telemetry', 'data', 'training', '*'))
    tst_channel_paths = glob.glob(os.path.join(DATA_DIRS, 'data', 'telemetry', 'data', 'testing', '*'))
